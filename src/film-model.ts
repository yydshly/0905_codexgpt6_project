...
export const FILM_STORAGE = 'ideal-study.film.v4';
export const FPS = 30;
...
export interface FilmProject { app: 'ideal-study-film'; version: 4; name: string; scene: Plan; film: { version: 1; shots: Shot[] }; selectedShotId: string; playhead: number }
...
export function createFilmProject(scene = initialPlan()): FilmProject {
  return { app: 'ideal-study-film', version: 4, name: '光落书房 · 十秒日常', scene: clone(scene), film: { version: 1, shots: [
...
export function parseFilmProject(raw: unknown): { project: FilmProject; migrated: boolean } {
  ...
  if ((raw as Plan).app === 'ideal-study' && [1,2,3].includes((raw as Plan).version)) {
  ...
  if (p.app !== 'ideal-study-film' || ![2,3,4].includes(p.version) || typeof p.name !== 'string' || !p.name.trim() || p.name.length > 48 || p.film?.version !== 1 || !Array.isArray(p.film.shots) || p.film.shots.length < 1 || p.film.shots.length > 3) throw new Error('请导入 v2/v3/v4 短片工程或 v1/v2/v3 书房方案。');
  ...
  return { migrated: p.version !== 4, project: { app: 'ideal-study-film', version: 4, name: p.name.trim(), scene, film: { version: 1, shots: p.film.shots.map(s => ({ id: s.id, name: s.name.trim(), duration: s.duration, start: { ...s.start }, end: { ...s.end }, target: [...s.target] })) }, selectedShotId: p.selectedShotId, playhead: Math.round(p.playhead * FPS) / FPS } };
}
...
export function loadFilmProject(): { project: FilmProject; restored: boolean; message: string } {
  try {
    ...
    const previous=localStorage.getItem('ideal-study.film.v3') ?? localStorage.getItem('ideal-study.film.v2');
    ...
    const legacy = localStorage.getItem(ROOM_STORAGE) ?? localStorage.getItem('ideal-study.plan.v2') ?? localStorage.getItem('ideal-study.plan.v1');
