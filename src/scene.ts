...
export interface SceneCallbacks { select(id:string|null):void; begin():void; move(id:string,x:number,z:number,y?:number):void; end():void; camera():void; error(message:string):void }
export class StudyScene {
  ...
  private drag:{id:string,startX:number,startY:number,offset:T.Vector3,plane:T.Plane,wall:boolean,moved:boolean}|null=null;
  ...
  constructor(private host:HTMLElement, private cb:SceneCallbacks) {
    ...
    this.observer=new ResizeObserver(()=>this.resize());this.observer.observe(host);this.resize();this.loop();
  }
  ...
  sync(plan:Plan){
    ...
    for(const o of plan.objects){
      const key=JSON.stringify([o.kind,o.material,o.width,o.height,o.photo]);let g=this.groups.get(o.id);
      ...
      g.position.set(o.x,o.kind==='wallPhoto'?o.y!:o.parentId?.78:['chair','sofa','bed'].includes(o.kind)&&this.onRug(o,plan)?.027:0,o.z);g.rotation.y=o.rotation*Math.PI/180;
    ...
    const shadowKey=JSON.stringify([plan.mood,plan.objects.map(o=>[o.id,o.kind,o.material,o.x,o.z,o.y,o.width,o.height,o.rotation,o.parentId])]);
  ...
  private disposeUnique(g:T.Group){g.traverse(c=>{if(c instanceof T.InstancedMesh)c.dispose();if(c instanceof T.Mesh&&!Array.isArray(c.material)){if(c.material.name==='userPhoto'){c.material.map?.dispose();c.geometry.dispose();c.material.dispose();}else if(c.material.name==='bulb'||c.material.emissiveIntensity===.35)c.material.dispose();}});}
  updateSelection(){
    const o=this.plan?.objects.find(o=>o.id===this.plan.selectedId);
    const key=JSON.stringify([o?.id,o?.kind,o?.width,o?.height,this.plan?.mood]);
    if(key===this.selectionKey){if(o){this.selection.position.set(o.x,o.kind==='wallPhoto'?o.y!:0,o.z);this.selection.rotation.y=o.rotation*Math.PI/180;}this.positionSelectionLabel();return;}
    ...
    const def=dimensions(o),y=o.parentId?.786:.04;
    if(o.kind==='wallPhoto') {
      const geo=new T.EdgesGeometry(new T.BoxGeometry(def.width+.025,def.height+.025,.047));
      const line=new T.LineSegments(geo,new T.LineBasicMaterial({color:this.plan.mood==='night'?'#c9e6ab':'#49734e',depthTest:false}));line.renderOrder=100;
      this.selection.add(line);this.selection.position.set(o.x,o.y!,o.z+.009);this.selection.rotation.y=0;this.positionSelectionLabel();return;
    }
  ...
  private positionSelectionLabel(){
    ...
    const p=group.position.clone();p.y+=dimensions(item).height*(item.kind==='wallPhoto'?.5:1)+.14;p.project(this.camera);
  ...
  private pointerDown=(e:PointerEvent)=>{
    ...
    const item=this.plan.objects.find(o=>o.id===id)!,wall=item.kind==='wallPhoto',y=item.parentId?.78:0,p=new T.Vector3();
    const plane=wall?new T.Plane(new T.Vector3(0,0,1),-item.z):new T.Plane(new T.Vector3(0,1,0),-y);
    if(!this.ray.ray.intersectPlane(plane,p))return;
    this.drag={id,startX:e.clientX,startY:e.clientY,offset:new T.Vector3(item.x,wall?item.y!:0,item.z).sub(p),plane,wall,moved:false};this.controls.enabled=false;this.renderer.domElement.setPointerCapture(e.pointerId);
  };
  private pointerMove=(e:PointerEvent)=>{
    ...
    if(!d.moved){this.cb.begin();d.moved=true;}this.cast(e);const p=new T.Vector3();if(this.ray.ray.intersectPlane(d.plane,p)){p.add(d.offset);this.cb.move(d.id,p.x,p.z,d.wall?p.y:undefined);}this.renderer.domElement.style.cursor='grabbing';
  };
  ...
  async exportGLB(selectedOnly=false) {
    const { exportSceneGLB }=await import('./model-export');
    const selected=this.plan.objects.find(o=>o.id===this.plan.selectedId);
    if(selectedOnly&&!selected)throw new Error('请先选中要导出的物件。');
    return exportSceneGLB(selectedOnly?[this.groups.get(selected!.id)!]:[this.room,...this.groups.values(),this.sun,this.fill],selectedOnly?selected!.id:this.plan.name,selectedOnly);
  }
  destroy(){cancelAnimationFrame(this.frame);this.observer.disconnect();this.controls.dispose();for(const g of this.groups.values())this.disposeUnique(g);this.composer.dispose();this.renderer.dispose();this.renderer.domElement.remove();this.selectionLabel.remove();}
}
