...
import { createIcons, Box, ChevronDown, ArrowUpRight, Undo2, Redo2, Download, Save, MousePointer2, Orbit, ZoomIn, ZoomOut, RotateCcw, Move, RotateCw, Trash2, X, Sun, Sunset, Moon, Check, Plus, SlidersHorizontal, Layers3, Grid2X2, Maximize, Focus, Upload, Image, FileJson, ArrowLeft, Lightbulb, LockKeyhole, Info, Command, CheckCheck, Leaf, PanelLeftClose, Table2, Armchair, Monitor, LibraryBig, LampDesk, LampFloor, Sprout, RectangleHorizontal, Pencil, BedDouble } from 'lucide';
import { ROOM_STORAGE, MAX_JSON_BYTES, dimensions, CATALOG, MATERIALS, CAMERA_LIMITS, displayName, clone, initialPlan, createItem, parsePlan, updateItem, type Plan, type Kind, type Mood } from './model';
...
$('#app').innerHTML=`
  ...
  <dialog id="export-dialog"><div class="dialog-head"><div><span class="eyebrow">KEEP YOUR INSPIRATION</span><h2>留住这一刻的理想</h2></div><button id="close-dialog" class="icon-btn" aria-label="关闭导出">${icon('X')}</button></div><p>把布置分享成图片，或将方案带到另一个浏览器。</p><button class="export-option" id="export-png"><span class="export-icon">${icon('Image')}</span><span><strong>导出空间图片</strong><small>PNG · 2 倍清晰度 · 仅含当前 3D 画面</small></span>${icon('ArrowUpRight')}</button><button class="export-option" id="export-json"><span class="export-icon">${icon('FileJson')}</span><span><strong>导出可编辑方案</strong><small>JSON · 保留物件、材质、灯光与镜头</small></span>${icon('ArrowUpRight')}</button><button class="export-option" id="export-glb"><span class="export-icon">${icon('Box')}</span><span><strong>导出整间房屋模型</strong><small>GLB · 米制 · 含家具、材质和照片</small></span>${icon('ArrowUpRight')}</button><button class="export-option" id="export-item-glb"><span class="export-icon">${icon('Box')}</span><span><strong>导出选中物件模型</strong><small>GLB · 物件置于原点 · 请先选中物件</small></span>${icon('ArrowUpRight')}</button><a class="export-option" href="?workspace=viewer" target="_blank" rel="noopener"><span class="export-icon">${icon('Orbit')}</span><span><strong>打开独立模型查看器</strong><small>检查实际导出的 GLB · 旋转观察</small></span>${icon('ArrowUpRight')}</a><a class="reuse-example-link" href="?workspace=integration" target="_blank" rel="noopener">网页二次开发：打开可操作接入示例 →</a><div class="dialog-note">GLB 保留模型与贴图；编辑逻辑、短片时间轴及后期效果请复用工程与源码。</div><div class="import-divider"><span>或继续已有的灵感</span></div><button class="button import-button" id="import-json">${icon('Upload')}导入 JSON 方案</button><div class="dialog-note">${icon('Info')}导入会替换当前画面，可通过「撤销」恢复。</div></dialog>
...
function photoPanel(o:import('./model').Item){return `<section class="property-section"><div class="section-label"><span>${icon('Image')}照片与画框</span><small>背墙悬挂 / m</small></div><div class="photo-preview">${o.photo?`<img src="${o.photo}" alt="当前墙面照片"/>`:'<span>几何装饰画 · 可上传自己的照片</span>'}</div><div class="rotate-controls"><button id="photo-upload">${icon('Upload')}${o.photo?'替换照片':'上传照片'}</button><button id="photo-reset" ${o.photo?'':'disabled'}>恢复装饰画</button></div><p class="material-description">JPG / PNG / WebP，最大 8 MB。照片随工程保存，保留原比例。</p><div class="photo-fields">${[['x','X 位置',o.x],['y','悬挂高度',o.y],['width','相框宽度',o.width],['height','相框高度',o.height]].map(([key,label,value])=>`<label>${label}<input id="pos-${key}" aria-label="${label}" type="number" step="0.1" value="${Number(value).toFixed(2)}"/></label>`).join('')}</div><p class="material-description">拖动画框沿墙移动 · 方向键调整位置</p></section>`;}
function renderPanel(){
  ...
  for(const field of (o.kind==='wallPhoto'?['x','y','width','height']:['x','z']) as ('x'|'y'|'z'|'width'|'height')[]){const input=$<HTMLInputElement>('#pos-'+field);input.onchange=()=>{const n=Number(input.value);if(!Number.isFinite(n)||input.value.trim()===''){toast('请输入有效的数值。',true);renderPanel();return;}mutate(()=>updateItem(plan,o.id,{[field]:n},true));};}
  if($('#rotation'))$<HTMLInputElement>('#rotation').onchange=e=>{const input=e.target as HTMLInputElement,n=Number(input.value);if(!Number.isFinite(n)||!input.value.trim()){toast('请输入有效的角度。',true);renderPanel();return;}mutate(()=>updateItem(plan,o.id,{rotation:n}));};
  if($('#rotate-left'))$('#rotate-left').onclick=()=>rotate(-15);if($('#rotate-right'))$('#rotate-right').onclick=()=>rotate(15);
  if(o.kind==='wallPhoto'){ $('#photo-upload').onclick=()=>{$<HTMLInputElement>('#photo-input').dataset.itemId=o.id;$<HTMLInputElement>('#photo-input').click();};$('#photo-reset').onclick=()=>mutate(()=>updateItem(plan,o.id,{photo:undefined})); }
...
for(const [id,selected] of [['export-glb',false],['export-item-glb',true]] as const)$('#'+id).onclick=async()=>{
  const b=$<HTMLButtonElement>('#'+id);b.disabled=true;
  try{if(!scene)throw new Error('3D 渲染不可用。');toast('正在打包模型、材质与照片…');download(await scene.exportGLB(selected),'glb');toast('GLB 模型已生成，已交给浏览器下载。可用独立查看器检查。');}catch(e){toast((e as Error).message,true);}finally{b.disabled=false;}
};
