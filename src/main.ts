...
function updateCameraUI(){if(!scene)return;const view=scene.getView();document.querySelectorAll<HTMLElement>('[data-view]').forEach(b=>{const active=b.dataset.view===view;b.classList.toggle('active',active);b.setAttribute('aria-pressed',String(active));});$('#zoom-in').toggleAttribute('disabled',scene.camera.zoom>=2.4);$('#zoom-out').toggleAttribute('disabled',scene.camera.zoom<=.55);}
