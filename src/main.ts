import './style.css';
import { createIcons, Box, ChevronDown, ArrowUpRight, Undo2, Redo2, Download, Save, MousePointer2, Orbit, ZoomIn, ZoomOut, RotateCcw, Move, RotateCw, Trash2, X, Sun, Sunset, Moon, Check, Plus, SlidersHorizontal, Layers3, Grid2X2, Maximize, Focus, Upload, Image, FileJson, ArrowLeft, Lightbulb, LockKeyhole, Info, Command, CheckCheck, Leaf, PanelLeftClose, Table2, Armchair, Monitor, LibraryBig, LampDesk, LampFloor, Sprout, RectangleHorizontal } from 'lucide';
import { CATALOG, MATERIALS, clone, initialPlan, createItem, parsePlan, updateItem, type Plan, type Kind, type Mood } from './model';
import { StudyScene } from './scene';

const ICONS={Box,ChevronDown,ArrowUpRight,Undo2,Redo2,Download,Save,MousePointer2,Orbit,ZoomIn,ZoomOut,RotateCcw,Move,RotateCw,Trash2,X,Sun,Sunset,Moon,Check,Plus,SlidersHorizontal,Layers3,Grid2X2,Maximize,Focus,Upload,Image,FileJson,ArrowLeft,Lightbulb,LockKeyhole,Info,Command,CheckCheck,Leaf,PanelLeftClose,Table2,Armchair,Monitor,LibraryBig,LampDesk,LampFloor,Sprout,RectangleHorizontal};
...
$('#app').innerHTML=`
  ...
  <main class="workspace">
    ...
    <section class="stage" aria-label="三维空间编辑区">
      ...
      <div class="mode-switch" role="group" aria-label="操作模式"><button id="edit-mode" class="active" title="选择与移动 V">${icon('MousePointer2')}<span>布置</span><kbd>V</kbd></button><button id="orbit-mode" title="旋转观察 C">${icon('Orbit')}<span>观察</span><kbd>C</kbd></button></div>
...
function setMode(mode:'edit'|'orbit'){scene?.setMode(mode);$('#edit-mode').classList.toggle('active',mode==='edit');$('#orbit-mode').classList.toggle('active',mode==='orbit');$('#stage-hint').innerHTML=mode==='edit'?`${icon('MousePointer2')}点击选择 · 拖动物件移动<span>右键拖动观察 · 滚轮缩放</span>`:`${icon('Orbit')}拖动旋转观察 · 滚轮缩放<span>切回「布置」即可编辑物件</span>`;icons();}
...
$('#reset-view').onclick=()=>{scene?.view('default');document.querySelectorAll<HTMLElement>('[data-view]').forEach(b=>b.classList.toggle('active',b.dataset.view==='default'));};
document.querySelectorAll<HTMLElement>('[data-view]').forEach(b=>b.onclick=()=>{scene?.view(b.dataset.view as 'default'|'top'|'close');document.querySelectorAll('[data-view]').forEach(x=>x.classList.toggle('active',x===b));});
...
document.addEventListener('keydown',e=>{
  const editing=(e.target as HTMLElement).matches('input,textarea,[contenteditable]');
...
Object.defineProperty(window,'__study',{value:{getPlan:()=>clone(plan),history:()=>({past:past.length,future:future.length}),metrics:()=>scene?.getMetrics(),project:(id:string)=>{const group=scene?.groups.get(id);if(!group||!scene)return null;const v=group.position.clone();v.y+=CATALOG[plan.objects.find(o=>o.id===id)!.kind].height*.5;v.project(scene.camera);const r=scene.renderer.domElement.getBoundingClientRect();return {x:r.left+(v.x+1)/2*r.width,y:r.top+(1-v.y)/2*r.height};},getCamera:()=>scene?.getCamera()},writable:false});
