...
import { parseGuideProject } from './guide-model';
...
try{
  ...
  if(raw?.app==='ideal-study-guide') {
    const { mountGuide } = await import('./guide-view');
    await mountGuide(parseGuideProject(raw),{visitor:true});
  } else await mountPortfolioView(parseFilmProject(raw).project,{visitor:true});
