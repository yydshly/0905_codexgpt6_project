...
export function halfBounds(item: Item) {
  const c = CATALOG[item.kind], a = item.rotation * Math.PI / 180;
...
export function createItem(plan: Plan, kind: Kind): Item {
  ...
  for (let z=.8;z>=-1.9;z-=.5) for (let x=0;x<=2.5;x+=.5) for (const sign of [1,-1]) {
    ...
    if(kind==='rug'||!plan.objects.some(o=>!o.parentId&&o.kind!=='rug'&&Math.abs(o.x-candidate.x)<halfBounds(o).x+b.x+.06&&Math.abs(o.z-candidate.z)<halfBounds(o).z+b.z+.06)) return candidate;
  }
...
export function parsePlan(raw: unknown): Plan {
  ...
  const p = raw as Plan;
  if(p.app!=='ideal-study'||p.version!==1||typeof p.name!=='string'||!p.name.trim()||p.name.length>48||!['day','dusk','night'].includes(p.mood)||!Array.isArray(p.objects)||p.objects.length>40) throw new Error('无法识别此方案，请导入理想书房导出的 v1 JSON 文件。');
