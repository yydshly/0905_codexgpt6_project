...
export async function createStudyPlayer(host:HTMLElement,initial:unknown=createFilmProject()) {
  ...
  const pause=()=>{playing=false;cancelAnimationFrame(frame);scene.setInteractionEnabled(true);};
  const render=()=>{scene.sync({...project.scene,selectedId:null});draw(project.playhead);};
  ...
  return {
    ...
    setMaterial(id:string,value:string){active();const item=project.scene.objects.find(o=>o.id===id);if(!item||!Object.hasOwn(MATERIALS,value)||!CATALOG[item.kind].materials.includes(value))throw new Error('物件或材质不受支持。');item.material=value;scene.sync({...project.scene,selectedId:null});},
    ...
    play(){active();pause();if(project.playhead>=totalDuration(project))draw(0);playing=true;scene.setInteractionEnabled(false);offset=project.playhead;start=performance.now();frame=requestAnimationFrame(tick);},
    pause,
