...
try {
  ...
  let connectedOrigin:string|undefined;
  player.onActivate(detail=>{if(connectedOrigin)parent.postMessage({channel:'ideal-study',version:1,event:'activateProject',detail},connectedOrigin);});
  ...
  window.addEventListener('message',async e=>{
    ...
    try {
      switch(command){
        ...
        case 'getState':break;
        case 'setMarkersVisible':player.setMarkersVisible(payload);break;
        case 'resetView':player.resetView();break;
        default:throw new Error('不支持的播放器指令。');
      }
      connectedOrigin=e.origin;
