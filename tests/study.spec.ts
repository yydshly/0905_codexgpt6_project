...
test('完整验收：添加、拖动、旋转、灯光、材质、历史、本地恢复、PNG、JSON',async({page,browser})=>{
  ...
  steps.push({step:'材质/深夜/撤销重做/保存刷新恢复',keyDataExact:true,storedBytes:await page.evaluate(()=>localStorage.getItem('ideal-study.plan.v3')!.length)});
...
test('异常镜头和物件名称导入保留方案，旧 v1 文件仍可编辑',async({page})=>{
  await ready(page);await page.getByRole('button',{name:'保存方案',exact:true}).click();const before=await plan(page),h=await history(page),stored=await page.evaluate(()=>localStorage.getItem('ideal-study.plan.v3'));
  await page.getByRole('button',{name:'导出',exact:true}).click();
