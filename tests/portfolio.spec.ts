...
test('作品错误、取消关联与历史、旧版房间和短片迁移',async({page})=>{
  ...
  await page.locator('#file-input').setInputFiles('docs/reuse-evidence/reusable-room.json');await expect(page.locator('#toast')).toContainText('已导入');const migrated=await plan(page);expect(migrated.version).toBe(3);expect(migrated.portfolio).toEqual({version:2,presentation:{name:'我的创作书房',headline:'在日常里，\n遇见我的作品。',bio:'一个房间，许多创作。\n点击物品，认识我的作品。'},projects:[],bindings:[]});
