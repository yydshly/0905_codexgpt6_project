...
test('导览闭环：修改、历史、保存恢复、JSON、真实视频重新播放与同帧比对', async ({ page, browser }) => {
  ...
  const info = await page.locator('#guide-video').evaluate((v: HTMLVideoElement) => ({ duration: v.duration, width: v.videoWidth, height: v.videoHeight, ready: v.readyState })); expect(info.duration).toBeCloseTo(14.5, 2); expect([info.width, info.height]).toEqual([1280, 720]);
  ...
  try {
    ...
    for (const time of [1.5, 5.5, 10, 13]) { await page.bringToFront(); await scrub(page, time); const preview = await pixels(page, '#guide-canvas canvas'); await replay.bringToFront(); await replay.locator('video').evaluate((v: HTMLVideoElement, t) => new Promise<void>(resolve => { v.onseeked = () => resolve(); v.currentTime = t; }), time); const mae = difference(preview, await pixels(replay, 'video')); expect(mae).toBeLessThan(9); frameChecks.push({ time, meanAbsolutePixelDifference: mae }); }
    await capture(replay, '05-downloaded-guide-opened.png'); await replay.locator('video').evaluate((v: HTMLVideoElement) => { v.currentTime = 0; return v.play(); }); await replay.waitForFunction(() => document.querySelector('video')!.ended, {}, { timeout: 22000 }); expect(await replay.locator('video').evaluate((v: HTMLVideoElement) => v.error)).toBeNull();
...
test('导览网站包离线于编辑器、子路径部署、作品关联与访客模式', async ({ page, browser }) => {
  ...
  expect(files['assets/creator-18.glb']).toBeUndefined();
...
test('青年骨骼、姿态重复采样、近景、旧角色迁移及模型加载失败保护', async ({ page }) => {
  ...
  const migrated = await project(page); expect(migrated).toEqual({ ...before, guide: { ...before.guide, avatar: 'creator-18-v1' } }); expect(migrated.guide.version).toBe(3);
...
test('个人 IP 切换事务、v2 外观保留、撤销重做与旧角色网站资源', async ({ page, browser }) => {
  ...
  legacy.guide.version = 2; legacy.guide.avatar = 'creator-18-v1'; legacy.guide.name = '旧工程角色';
  ...
  expect(await project(page)).toEqual({ ...legacy, guide: { ...legacy.guide, version: 3 } });
  await page.locator('#guide-avatar').selectOption('personal-creator-01-v1');
