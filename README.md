...
本分支 `codex/character-guide` 新增「小禾角色导览」可运行原型：以 18 岁青年为基准的骨骼角色、绕开家具行走、阅读随身手册与屏幕介绍、两段时间轴、设置与历史、本地导览快照、JSON、真实视频和独立导览网站包。默认 16 秒，新导览使用个人 IP 设定 01 的 3D 样版，支持与旧角色切换、保存及发布；保留原有全部工作区。[个人 IP 3D 接入样版](docs/character-ip/INTEGRATION.md) · [青年角色升级与验收](docs/ADULT-AVATAR.md) · [上一版原型记录](docs/CHARACTER-GUIDE.md)。此分支尚未合并或部署到线上主站。
