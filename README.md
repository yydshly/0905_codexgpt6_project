...
默认入口是短片工作台；`/?workspace=room` 是原 v1 空间布置器。两页顶部都有工作区导航：短片页点击「1 布置书房」，即可编辑当前短片的房间；书房页点击「2 返回短片工作台」，会先保存房间，再返回原镜头工程。绿色标签表示当前位置。保存失败会留在原页提示，不丢弃改动。原左下「保存并布置房间」入口也保留。
