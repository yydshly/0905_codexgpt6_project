 ...
 * npm run dev, then node scripts/review-personal-avatar.mjs
 * Uses a fresh isolated Chrome context and does not touch the user's saved projects. */
import { chromium } from '@playwright/test';
import { mkdir, writeFile } from 'node:fs/promises';
const destination = process.env.GUIDE_EVIDENCE_DIR ?? 'docs/character-02-evidence';
...
try {
  ...
  await page.evaluate(async () => {
    ...
    const actors = await Promise.all(['personal-creator-01-v1', 'personal-creator-02-v1'].map(createGuideCharacter));
    ...
    window.review = (edition = 1, angle = 0, mode = 'portrait') => {
      ...
      const portrait = mode !== 'full', distance = portrait ? 1.1 : 3.6;
      camera.position.set(Math.sin(angle) * distance, mode === 'above' ? 2.35 : portrait ? 1.66 : 1.22, Math.cos(angle) * distance);
      camera.lookAt(0, portrait ? 1.58 : .92, 0); renderer.render(scene, camera);
